"""
scripts/generate_invoices.py  —  Step 1 of the invoice-ocr-pipeline
=====================================================================
...
Phase 2 additions:
  --export-truth flag: writes data/ground_truth_invoices.csv with every
  invoice's intended field values, enabling validate_accuracy.py to compare
  OCR extraction results against what was actually generated.
"""
...
def generate_invoice(invoice_number: int, output_dir: Path) -> dict:
    """
    Generate one PDF invoice.

    Returns a dict with the ground-truth values used to build the PDF,
    enabling downstream accuracy validation.

    Raises on any I/O or rendering error.
    """
    ...
    normal_style = ParagraphStyle("Normal9", parent=styles["Normal"], fontSize=9)
    story = []
    ...
    # Return ground truth record for --export-truth
    return {
...
def main(export_truth: bool = False) -> None:
    ...
    for i in range(1, INVOICE_COUNT + 1):
        try:
            record = generate_invoice(i, OUTPUT_DIR)
    ...
    if export_truth and ground_truth_records:
        ...
        fieldnames = ["invoice_id", "invoice_date", "vendor_name",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--export-truth",
        action="store_true",
        help="Write data/ground_truth_invoices.csv with intended field values "
             "(used by validate_accuracy.py)",
    )
    args = parser.parse_args()
    main(export_truth=args.export_truth)